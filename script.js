    ...
    for (const date in data.events) {
      data.events[date].forEach((event, idx) => {
        ...
        row.innerHTML = `
          ...
          <td>${event.match}</td>
          <td><a class="watch-btn" target="_blank" href=""C:\Users\PC\Desktop\Match website\my-stream-site\New folder\Streaming-site\watch.html"?id=${event.unix_timestamp}_${idx}">Watch</a></td>
        `;
